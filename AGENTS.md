...
- 推荐视图：`frontend/src/components/RecommendationView.vue`
- 配件库视图：`frontend/src/components/CatalogView.vue`
- 管理视图：`frontend/src/components/AdminView.vue`
- 推荐逻辑：`frontend/src/composables/useRecommendation.js`
...
## 3. 已发现问题与处理结论
### 访问与路由
- 生产环境 `/admin` 访问失败：需服务端做 SPA 回退（指向 `index.html`）
...
- 主板“芯片组≠品牌”：避免通用项误用，改为具体型号

### 数据与规则
- 50 系显卡：使用占位型号补齐（5050/5060/5060 Ti/5070/5070 Ti/5080/5090）
...
- 图标/微标：先复制 SVG 到 `.superdesign/design_iterations` 再编辑
...
- 排版：至少 H1/H2/正文三级，差异明显；正文 ≥ 16px

## 6. 仓库专属约束
- 前后端拆分清晰：`frontend/` 为前端、`backend/` 为后端
- 数据源：`data/` 为真源；`frontend/public/data/` 为前端兜底
- 数据库：优先 PostgreSQL；除非明确要求，不使用 SQLite
- 规则/配件更新：修改 `data/components.json` 与 `data/rules.json`，同步到前端或经后端接口写入

## 7. 用户工作流覆盖
- 用户输入 `ulw` 时，加载 `D:\workspace\ultrawork.md` 并按其中指令执行
